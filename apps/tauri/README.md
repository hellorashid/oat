...
Cross-platform menubar recorder. Pick a local folder, start a recording from the tray, and Oat saves the audio plus a markdown transcript on your machine.
...
Install from the repo root so the workspace lockfile stays in sync:
...
Or package a local macOS `.app` + `.dmg`:
